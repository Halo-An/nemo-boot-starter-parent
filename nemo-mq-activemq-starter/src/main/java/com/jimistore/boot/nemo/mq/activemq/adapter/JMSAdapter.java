...
public class JMSAdapter implements IMQAdapter {
	...
	public JMSAdapter init() {
		...
		activeMQConnectionFactory = new ActiveMQConnectionFactory(myActiveMQProperties.getUser(),
				myActiveMQProperties.getPassword(), myActiveMQProperties.getBrokerUrl());
		...
		if (myActiveMQProperties.getPool() != null) {
			JmsPoolConnectionFactoryProperties pool = myActiveMQProperties.getPool();
			pooledConnectionFactory.setBlockIfSessionPoolIsFull(pool.isBlockIfFull());
			pooledConnectionFactory.setBlockIfSessionPoolIsFullTimeout(pool.getBlockIfFullTimeout().getNano() / 1000);
			pooledConnectionFactory.setIdleTimeout(pool.getIdleTimeout().getNano() / 1000);
			pooledConnectionFactory.setMaxConnections(pool.getMaxConnections());
			pooledConnectionFactory
					.setTimeBetweenExpirationCheckMillis(pool.getTimeBetweenExpirationCheck().getNano() / 1000);
