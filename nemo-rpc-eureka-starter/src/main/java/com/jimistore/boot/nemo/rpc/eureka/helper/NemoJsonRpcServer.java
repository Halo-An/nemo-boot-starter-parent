...
public class NemoJsonRpcServer extends JsonRpcServer {
	...
	public NemoJsonRpcServer(ObjectMapper mapper, Object handler,
	...
	protected JsonNode invoke(Object target, Method m, List<JsonNode> params)
		...
		for (int i = 0; i < parameterTypes.length; i++) {
			...
			JavaType paramJavaType = TypeFactory.defaultInstance()
					.constructType(parameterTypes[i]);
			convertedParams[i] = mapper.readValue(paramJsonParser,
					paramJavaType);
		}
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String user = request.getHeader(Context.CONTEXT_REQUEST_USER);
		if(user!=null){
