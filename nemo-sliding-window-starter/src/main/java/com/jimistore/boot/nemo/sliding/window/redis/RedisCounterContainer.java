...
public class RedisCounterContainer extends LocalCounterContainer implements IRedisSyncTask {
	...
	private static final Logger LOG = LoggerFactory.getLogger(RedisCounterContainer.class);
	...
	public ICounterContainer createCounter(Topic topic) {
		...
		RedisCounter<?> counter = RedisCounter.create(slidingWindowProperties, redisTemplate, topic.getKey(),
				topic.getTimeUnit(), topic.getCapacity(), topic.getValueType());
		...
		// 同步给redis
		CounterMsg counterMsg = new CounterMsg().setCapacity(topic.getCapacity())
				.setKey(topic.getKey())
				.setClassName(topic.getClassName())
				.setTimeUnit(topic.getTimeUnitStr());

		try {
			boolean result = redisTemplate.opsForHash()
					.putIfAbsent(slidingWindowProperties.getRedisContainerKey(), topic.getKey(),
							objectMapper.writeValueAsString(counterMsg));
			if (result) {
				redisTemplate.expire(slidingWindowProperties.getRedisContainerKey(),
						slidingWindowProperties.getRedisExpired(), TimeUnit.MILLISECONDS);
			} else {
				counter.sync();
			}
			if (LOG.isDebugEnabled()) {
	...
	public void sync() {
		// 同步计数容器
		Map<String, String> src = redisTemplate.opsForHash().entries(slidingWindowProperties.getRedisContainerKey());
