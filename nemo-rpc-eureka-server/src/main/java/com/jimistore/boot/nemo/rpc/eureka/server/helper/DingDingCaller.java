...
public class DingDingCaller {
	...
	public void sendRobotNotice(String msg, String toRobot, String... toPhones){
		if(log.isDebugEnabled()){
			log.debug(String.format("call sendRobotNotice of dingding, the toRobot is %s, the msg is %s", toRobot, msg));
		}
		DingDingMsg dingDingMsg = new DingDingMsg()
				.setMsgtype("text")
				.setText(new Text().setContent(msg));
		...
		if(log.isDebugEnabled()){
			log.debug(String.format("call sendRobotNotice of dingding, the toRobot is %s, the msg is %s, the result is %s", toRobot, msg, result));
		}
