...
public class InitContextAspect {
	...
	private static final Logger LOG = LoggerFactory.getLogger(InitContextAspect.class);
	...
	private void initBaseBean(BaseBean<?> baseBean, Date now, String user, Set<BaseBean<?>> set) {
		...
		for (Field field : fields) {
			...
			String getMethodName = String.format("%s%s%s", prefix, field.getName().substring(0, 1).toUpperCase(),
