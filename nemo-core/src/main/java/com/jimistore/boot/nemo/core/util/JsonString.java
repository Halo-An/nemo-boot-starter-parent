...
//			List<Field> fields=getFields();
			...
			for (Field field : fields) {
				...
				if (json != null) {
					...
					JsonTitleAlias jsonTitleAlias = (JsonTitleAlias) AnnotationUtil.getAnnotation(field,
							JsonTitleAlias.class);
						...
						if (value != null) {
							if (field.getType().isArray() || value.getClass().isArray()) {
								...
								result = new StringBuilder(
										new JsonList(getSonFields(fieldNames, key), list, exclusionMap).toString());
								result.append(",");
							} else {
								result = new StringBuilder();
								if (ClassUtil.isBasicType(value) || value instanceof String
										|| value instanceof java.util.Date) {
									result.append("\"");
								}
								result.append(
										new JsonString(value, getSonFields(fieldNames, key), exclusionMap).toString());
		...
		if (fieldNames != null) {
			for (int i = 0; i < fieldList.size(); i++) {
				...
				for (String fieldName : fieldNames) {
					if (field.getName()
							.equals(fieldName.indexOf(".") > 0 ? fieldName.substring(0, fieldName.indexOf("."))
									: fieldName)) {
						flag = false;
					}
				}
